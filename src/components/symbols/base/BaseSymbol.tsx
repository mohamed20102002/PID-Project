...
const renderPath = (
  ...
  switch (path.type) {
    ...
    case 'arc': {
      const { cx, cy, r, startAngle, endAngle } = path.data as {
        cx: number;
        cy: number;
        r: number;
        startAngle: number;
      ...
      for (let i = 0; i <= steps; i++) {
        ...
        points.push(cx * width + radius * Math.cos(angle));
        points.push(cy * height + radius * Math.sin(angle));
      }
...
export const BaseSymbol: React.FC<BaseSymbolProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Render labels (skip for noKks symbols) */}
      {!definition.noKks && definition.labels?.map((label, index) =>
